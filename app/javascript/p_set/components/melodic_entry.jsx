...
export default class MelodicEntryComponent extends React.Component {
  ...
  render() {
    ...
    const solfege = this.props.options.solfege
      .filter(([_, v]) => v)
      .map(([v, _]) => v)
      .reverse()
    ...
    const keyCorrect =
      keySignature ===
      getVFScaleName(this.props.stave.tonic, this.props.stave.scale)
    ...
    return (
      <div className="row columns" ref={(el) => (this.containerEl = el)}>
        <div className="reveal" id="help-text-melodic" data-reveal>
          <ul>
            <li>
              <b>Enter - </b> enters the current highlighted solfege
            </li>
            <li>
              <b>Space - </b> plays/pauses the audio
            </li>
            <li>
              <b>Right/Left - </b> arrows change current note
            </li>
            <li>
              <b>Up/Down - </b> raise or lower an octave
            </li>
            <li>
        ...
        <div className="row columns" style={showIf(!keyCorrect && !instructor)}>
          <fieldset>
            <legend>Key</legend>
            <select value={this.state.keySignature} onChange={this.updateKey}>
              {keyOptions}
            </select>
            <button
              className="button"
              onClick={(e) =>
                this.props.updateKeySignature(this.state.keySignature)
              }
            >
              Verify
            </button>
