...
export function measureLengths(key, staves) {
  ...
  return _.range(firstStave.length).map((i) => {
    return staves
      .map((stave) => stave[key])
      .reduce((max, stave) => {
...
export function compareMeters(meter, stave, pickUpBeat) {
  const regular = pickUpBeat ? _.slice(stave, 1, stave.length - 1) : stave
...
export function validatePickupBeat(meter, beat) {
  ...
  return (
    beat.top / beat.bottom < meter.top / meter.bottom &&
    beat.top > 0 &&
    beat.bottom > 0
  )
}
...
export function pickupOptions(meter, rhythmicOptions) {
  ...
  const denominator = _.chain(rhythmicOptions)
  ...
  const options = _.chain(numerators)
...
export function pickupRests(meter, pickupBeat) {
  const goal = new Fraction(meter.top, meter.bottom).sub(
    new Fraction(pickupBeat.top, pickupBeat.bottom)
  )
...
export function getAnswerErrors(solution, answer, mode) {
  const staves = _.zipWith(solution, answer, (s, a) => {
